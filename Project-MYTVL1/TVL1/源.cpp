...
#include "Metrics/EvalMetrics.h"
#include "Projection/Padded.h"
#include "Projection/Akaze.h"
...
int main(int argc, char* argv[])
{
	...
	//std::string moved_image_path = "D:\\Code-VS\\picture\\data\\dataset7-test1.jpg";
	//std::string fixed_image_path = "D:\\Code-VS\\picture\\data\\dataset7-test2.jpg";
	...
	// 对图片进行边缘填充，因后面射影变换可规定输出尺寸，故paaded可以删除
	ImagePadded(moved_image, fixed_image, moved_image, fixed_image);

	// 评价指标
	std::cout << "********************Padded Eval********************" << std::endl;
	std::cout << "MSE:" << MSE(fixed_image, moved_image) << std::endl;
	std::cout << "CC:" << CorrelationCoefficient(fixed_image, moved_image) << std::endl;
	std::cout << "PSNR:" << PSNR(fixed_image, moved_image) << std::endl;
	std::cout << "NMI:" << NormalizedMutualInformation(fixed_image, moved_image) << std::endl;
	std::cout << "MSSIM:" << MSSIM(fixed_image, moved_image) << std::endl;
	...
	std::cout << "moved_image: " << moved_image.size() << std::endl;
	std::cout << "fixed_image: " << fixed_image.size() << std::endl;
